...
9. **Dark Mode Support (Frontend Ownership — Assigned to Om)**: To improve accessibility for seniors and night-time medication checking, the application UI supports Dark Mode via CSS custom properties (`[data-theme="dark"]` or class toggle). Om is responsible for implementing the Dark Mode toggle switch and CSS theme variables across all frontend screens (`Navbar`, `Dashboard`, `UploadPage`, `CaregiverPage`). Backend API contracts remain completely theme-agnostic.
